...
def calculate_go_enrichment(my_go_counts, my_total_proteins, 
    ...
    df_results.to_csv("enrichment_results_updated.csv")
...
def expand_go_terms_with_ancestors(protein_go_data, go_obo):
    """
    Expand GO terms with their ancestors before counting.
    Args:
    ...
    Returns:
        tuple: (go_term_counts, total_proteins)
            - go_term_counts: Dictionary mapping GO terms (including ancestors) to their counts.
    ...
    for protein_id, data in protein_go_data.items():
        ...
        for category_terms in categories.values():
            for term_info in category_terms:
                ...
                # Count the GO term itself
                go_term_counts[go_id] += 1

                # Expand to include all ancestor terms
                if go_id in go_obo:
                    ancestors = go_obo[go_id].get_all_parents()
                    for ancestor in ancestors:
                        go_term_counts[ancestor] += 1
...
def analyze_go_hierarchy():
    ...
    df = pd.read_csv("enrichment_results_updated.csv")
    ...
    branches_df.to_csv('enriched_branches_updated.csv', index=False)
...
def main():
    ...
    print("Loading GO DAG for ancestor expansion...")
    go_obo = obo_parser.GODag('go.obo')

    go_counts_family, num_proteins_family = expand_go_terms_with_ancestors(family_annotations, go_obo)
    ...
    plt.savefig('go_enrichment_wordcloud_updated.png', dpi=300, bbox_inches='tight')
    plt.close()
